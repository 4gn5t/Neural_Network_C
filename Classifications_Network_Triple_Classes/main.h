...
void free_neuron(NeuronClassifier* neuron);
...
int fit(NeuronClassifier* neuron, Dataset* data, double alpha, double nu, double E0, int max_epochs);
...
void print_training_info(NeuronClassifier* neuron, int epochs, double train_error, double val_error);
void print_neuron_info(NeuronClassifier* neuron);
...
void print_neuron_info(NeuronClassifier* neuron) {
    printf("Neuron with %d classes and %d features\n", neuron->num_classes, neuron->num_features);
    
    for (int c = 0; c < neuron->num_classes; c++) {
        printf("Class %d weights: ", c);
        print_vector(neuron->weights[c], neuron->num_features);
        printf("Class %d bias: %.4f\n", c, neuron->bias[c]);
    }
...
void free_neuron(NeuronClassifier* neuron) {
    ...
    if (neuron->weights) {
        for (int c = 0; c < neuron->num_classes; c++) {
            free(neuron->weights[c]);
        }
        free(neuron->weights);
    }
    
    if (neuron->delta_weights_prev) {
        for (int c = 0; c < neuron->num_classes; c++) {
            free(neuron->delta_weights_prev[c]);
        }
        free(neuron->delta_weights_prev);
    }
    
    free(neuron->bias);
    free(neuron->delta_bias_prev);
    free(neuron->epochs);
...
void save_training_history(NeuronClassifier* neuron, const char* filename) {
    FILE* file = fopen(filename, "w");
    if (!file) {
